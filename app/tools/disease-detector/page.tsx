...
export default function DiseaseDetectorPage() {
  ...
  const handleFile = useCallback((file: File) => {
    if (file.size > 10*1024*1024) { setError(language==="bn"?"ছবি ১০MB এর বেশি নয়":"Image under 10MB only"); return }
  ...
  return (
    <div className="h-[calc(100vh-64px)] flex flex-col bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1"><h1 className="font-bold text-gray-900 text-sm">{language==="bn"?"ফসল রোগ সনাক্তকারী":"Crop Disease Detector"}</h1><p className="text-[11px] text-gray-400">{language==="bn"?"ছবি আপলোড বা লক্ষণ লিখুন":"Upload photo or describe"}</p></div>
        <div className="flex bg-gray-100 rounded-lg p-1">
          <button onClick={()=>setMode("image")} className={`px-3 py-1.5 rounded-md text-xs font-bold flex items-center gap-1.5 ${mode==="image"?"bg-white shadow text-gray-900":"text-gray-500"}`}><ImageIcon className="w-3.5 h-3.5"/>{language==="bn"?"ছবি":"Photo"}</button>
          <button onClick={()=>setMode("text")} className={`px-3 py-1.5 rounded-md text-xs font-bold flex items-center gap-1.5 ${mode==="text"?"bg-white shadow text-gray-900":"text-gray-500"}`}><FileText className="w-3.5 h-3.5"/>{language==="bn"?"লক্ষণ":"Text"}</button>
        </div>
      </div>
        ...
        <div className="max-w-2xl mx-auto p-4 md:p-6 space-y-3">
          {mode==="image"?(
              ...
              {image?(
                <div className="space-y-3">
                  ...
                  <div className="flex justify-center gap-2">
                    <button onClick={reset} className="btn-secondary-sm"><Trash2 className="w-4 h-4 mr-1"/>{language==="bn"?"সরান":"Remove"}</button>
                    <button onClick={analyze} disabled={loading} className="btn-primary-sm">{loading?<Loader2 className="w-4 h-4 mr-1 animate-spin"/>:<Microscope className="w-4 h-4 mr-1"/>{loading?t("tools.disease.analyzing"):t("tools.disease.analyze")}}</button>
                  </div>
                </div>
              ):(
                <div className="space-y-3">
                  ...
                  <div className="flex justify-center gap-2">
                    <button onClick={()=>fileInputRef.current?.click()} className="btn-primary-sm"><Upload className="w-4 h-4 mr-1"/>{language==="bn"?"ছবি নির্বাচন":"Choose"}</button>
                    <button onClick={()=>fileInputRef.current?.click()} className="btn-secondary-sm"><Camera className="w-4 h-4 mr-1"/>{t("tools.disease.camera")}</button>
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm space-y-3">
              ...
              <button onClick={analyze} disabled={loading||!description.trim()} className="btn-primary-sm w-full">{loading?<Loader2 className="w-4 h-4 mr-1 animate-spin"/>:<Microscope className="w-4 h-4 mr-1"/>{loading?t("tools.disease.analyzing"):t("tools.disease.analyze")}}</button>
            </div>
          )}
          ...
          {loading&&<div className="bg-white rounded-2xl border border-gray-100 p-8 text-center shadow-sm"><Loader2 className="w-8 h-8 text-leaf-600 animate-spin mx-auto mb-3"/><p className="font-semibold text-gray-700">{t("tools.disease.analyzing")}</p></div>}
