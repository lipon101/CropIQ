...
import { MessageCircle, Send, Loader2, Copy, Plus, History, X, Check, Trash2, Bot, User, ChevronRight } from "lucide-react"
...
export default function ChatbotPage() {
  ...
  // Intelligent scroll: only if user is near bottom
  useEffect(() => {
    const container = chatContainerRef.current
    if (!container) return
    const nearBottom = container.scrollHeight - container.scrollTop - container.clientHeight < 150
    if (nearBottom && chatEndRef.current) chatEndRef.current.scrollIntoView({ behavior: "auto" })
  }, [messages, loading])
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const res = await fetch("/api/chatbot", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: text.trim(), language, history }) })
      ...
      setMessages(prev => [...prev, { id: genId(), role: "assistant", content: language === "bn" ? "দুঃখিত, কিছু সমস্যা হয়েছে।" : "Sorry, something went wrong.", timestamp: new Date() }])
  ...
  const starterSuggestions = language === "bn"
    ? ["ধান গাছে ব্লাস্ট রোগের চিকিৎসা?", "আলু চাষের সঠিক সময়?", "টমেটো পাতা কুঁকড়ে যায় কেন?", "জৈব সার তৈরির পদ্ধতি?"]
    : ["How to treat rice blast?", "Best time to plant potatoes?", "Why tomato leaves curling?", "How to make organic fertilizer?"]
  ...
  return (
    <div className="h-[calc(100vh-64px)] flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1"><span className="font-bold text-gray-900 text-sm">{t("tools.chatbot.title")}</span><p className="text-[11px] text-gray-400">{language === "bn" ? "যেকোনো কৃষি প্রশ্ন করুন" : "Ask any farming question"}</p></div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"} fixed md:relative z-40 md:z-auto w-64 bg-white border-r border-gray-100 flex flex-col shrink-0 transition-transform duration-300 h-full`}>
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {userMessages.length === 0 ? <p className="text-xs text-gray-400 text-center py-10">{language === "bn" ? "কোন চ্যাট নেই" : "No chats yet"}</p>
              : userMessages.slice(-30).reverse().map(m => <button key={m.id} className="w-full text-left px-3 py-2 rounded-lg text-xs text-gray-600 hover:bg-leaf-50 truncate font-medium" onClick={() => setSidebarOpen(false)}>{m.content.slice(0, 45)}{m.content.length > 45 ? "…" : ""}</button>)}
          </div>
            ...
            {messages.length === 0 ? (
              <div className="flex items-center justify-center h-full">
                <div className="text-center max-w-sm">
                  ...
                  <p className="text-sm text-gray-500 mb-5">{language === "bn" ? "আমি আপনার কৃষি সহায়ক। যেকোনো প্রশ্ন করুন — বাংলা বা ইংরেজিতে।" : "I'm your farming assistant. Ask anything — in Bengali or English."}</p>
                  <div className="flex flex-wrap gap-2 justify-center">
                    {starterSuggestions.map((s, i) => <button key={i} onClick={() => sendMessage(s)} className="px-4 py-2 bg-gray-50 hover:bg-leaf-50 border border-gray-200 hover:border-leaf-200 rounded-xl text-xs font-medium text-gray-600 hover:text-leaf-700 transition-all">{s}</button>)}
                  </div>
                </div>
              </div>
            ) : (
              messages.map(msg => (
                <div key={msg.id} className={`flex gap-2.5 ${msg.role === "user" ? "justify-end" : ""}`}>
                  {msg.role === "assistant" && <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-blue-700 rounded-xl flex items-center justify-center shrink-0 shadow-sm mt-1"><Bot className="w-4 h-4 text-white" /></div>}
                  <div className={`max-w-[82%] ${msg.role === "user" ? "" : ""}`}>
                    {msg.role === "user" ? (
                      <div className="px-4 py-3 bg-leaf-600 text-white rounded-2xl rounded-br-md text-sm leading-relaxed whitespace-pre-line">{msg.content}</div>
                    ) : (
                      <div className="bg-gray-50 border border-gray-100 rounded-2xl rounded-bl-md px-4 py-3 text-sm leading-relaxed markdown-body">
                        <ReactMarkdown
                          ...
                          components={{
                            ...
                            th: ({ children }) => <th className="px-3 py-2 text-left font-bold text-gray-700 bg-gray-100 first:rounded-tl-lg last:rounded-tr-lg">{children}</th>,
                            ...
                            code: ({ children }) => <code className="bg-gray-200 px-1.5 py-0.5 rounded text-xs text-leaf-700 font-mono">{children}</code>,
                          }}
                        >
                          {msg.content}
                        </ReactMarkdown>
                        <button onClick={() => copyMessage(msg.content, msg.id)} className="text-[11px] text-gray-400 hover:text-leaf-600 flex items-center gap-1 mt-2 font-medium">
                  ...
                  {msg.role === "user" && <div className="w-8 h-8 bg-gradient-to-br from-leaf-500 to-leaf-700 rounded-xl flex items-center justify-center shrink-0 shadow-sm mt-1"><User className="w-4 h-4 text-white" /></div>}
                </div>
              ))
            )}
            {loading && (
              <div className="flex gap-2.5">
                ...
                <div className="bg-gray-50 border border-gray-100 rounded-2xl rounded-bl-md px-5 py-4"><div className="flex gap-1.5"><div className="w-2 h-2 bg-gray-300 rounded-full animate-bounce" /><div className="w-2 h-2 bg-gray-300 rounded-full animate-bounce [animation-delay:0.15s]" /><div className="w-2 h-2 bg-gray-300 rounded-full animate-bounce [animation-delay:0.3s]" /></div></div>
              </div>
            )}
            <div ref={chatEndRef} />
          </div>
          ...
          {suggestions.length > 0 && !loading && (
            <div className="px-4 py-2.5 border-t border-gray-100 bg-gray-50/50 shrink-0">
              ...
              <div className="flex flex-wrap gap-2">
                {suggestions.map((s, i) => (
                  <button key={i} onClick={() => sendMessage(s)} className="px-3.5 py-2 bg-white border border-gray-200 hover:border-leaf-300 hover:bg-leaf-50 rounded-xl text-xs font-medium text-gray-600 hover:text-leaf-700 transition-all flex items-center gap-1.5">
                    {s}
