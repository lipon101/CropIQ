  ...
  return (
    <div className="h-[calc(100vh-64px)] flex flex-col bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1"><span className="font-bold text-gray-900 text-sm">এআই কৃষি চ্যাটবট</span><p className="text-xs text-gray-400">যেকোনো কৃষি প্রশ্ন করুন</p></div>
        {messages.length > 0 && <button onClick={newChat} className="text-xs font-medium text-gray-400 hover:text-red-500 px-3 py-1.5 rounded-lg hover:bg-red-50">নতুন</button>}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"} fixed md:relative z-40 md:z-auto w-64 bg-white border-r border-gray-100 flex flex-col shrink-0 transition-transform duration-300 h-full`}>
          <div className="p-4 border-b border-gray-100">
            <div className="flex items-center justify-between mb-3"><h2 className="font-bold text-gray-800 text-sm flex items-center gap-2"><History className="w-4 h-4 text-leaf-600" />চ্যাট ইতিহাস</h2><button className="md:hidden p-1.5 hover:bg-gray-100 rounded-lg" onClick={() => setSidebarOpen(false)}><X className="w-4 h-4" /></button></div>
            <button onClick={newChat} className="btn-primary-sm w-full"><Plus className="w-3.5 h-3.5" />নতুন চ্যাট</button>
          </div>
          <div className="flex-1 overflow-y-auto p-2">{userMsgs.length === 0 ? <p className="text-xs text-gray-400 text-center py-10">কোন চ্যাট নেই</p> : userMsgs.slice(-30).reverse().map(m => <button key={m.id} className="w-full text-left px-3 py-2 rounded-lg text-xs text-gray-600 hover:bg-leaf-50 truncate font-medium" onClick={() => setSidebarOpen(false)}>{m.content.slice(0, 45)}{m.content.length > 45 ? "…" : ""}</button>)}</div>
          {messages.length > 0 && <div className="p-3 border-t border-gray-100"><button onClick={newChat} className="w-full text-xs text-red-500 hover:bg-red-50 py-2 rounded-lg flex items-center justify-center gap-1.5"><Trash2 className="w-3.5 h-3.5" />ক্লিয়ার করুন</button></div>}
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white">
          <div ref={chatContainerRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
            {messages.length === 0 ? (
              <div className="flex items-center justify-center h-full"><div className="text-center max-w-sm">
                <div className="w-14 h-14 bg-gradient-to-br from-blue-100 to-blue-200 rounded-2xl flex items-center justify-center mx-auto mb-4"><Bot className="w-7 h-7 text-blue-600" /></div>
                <h2 className="text-lg font-bold text-gray-800 mb-1">আসসালামু আলাইকুম! 🌱</h2>
                <p className="text-sm text-gray-500 mb-5">আমি আপনার কৃষি সহায়ক। যেকোনো কৃষি প্রশ্ন করুন — বাংলায়।</p>
                ...
                <div className="max-w-[82%]">
                  {m.role === "user" ? <div className="px-4 py-3 bg-leaf-600 text-white rounded-2xl rounded-br-md text-sm leading-relaxed whitespace-pre-line">{m.content}</div>
                    : <div className="bg-gray-50 border border-gray-100 rounded-2xl rounded-bl-md px-4 py-3 text-sm leading-relaxed markdown-body">
                      ...
                      <button onClick={() => copyMsg(m.content, m.id)} className="text-[11px] text-gray-400 hover:text-leaf-600 flex items-center gap-1 mt-2 font-medium">{copiedId === m.id ? <><Check className="w-3 h-3 text-green-500" />কপি হয়েছে</> : <><Copy className="w-3 h-3" />কপি</>}</button>
                    </div>}
                </div>
          ...
          {suggestions.length > 0 && !loading && (
            <div className="px-4 py-2.5 border-t border-gray-100 bg-gray-50/50 shrink-0">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">আরও জানতে চান?</p>
          ...
          <div className="p-3 border-t border-gray-100 bg-white shrink-0">
            <form onSubmit={e => { e.preventDefault(); send(input) }} className="flex gap-2">
              <input value={input} onChange={e => setInput(e.target.value)} placeholder="আপনার প্রশ্ন লিখুন..." className="flex-1 px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-leaf-500 focus:ring-2 focus:ring-leaf-100 outline-none text-sm" disabled={loading} />
              <button type="submit" disabled={loading || !input.trim()} className="btn-primary-sm !p-3 !px-3 !rounded-xl aspect-square"><Send className="w-5 h-5" /></button>
            </form>
          </div>
        </div>
      </div>
