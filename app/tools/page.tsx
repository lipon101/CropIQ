  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex flex-col">
      ...
      <div className="bg-white/90 backdrop-blur-xl border-b border-gray-100 px-4 md:px-8 py-2.5 flex items-center shrink-0 shadow-sm">
        ...
        <h1 className="text-sm md:text-base font-extrabold text-gray-800 mx-auto">সব টুল</h1>
        <div className="w-[68px]" />
      </div>

      {/* Content */}
      <div className="flex-1 px-4 sm:px-6 md:px-10 lg:px-16 py-8 md:py-12 max-w-4xl mx-auto w-full flex items-center">
        <div className="w-full space-y-8">
          {/* Section heading */}
          <div className="text-center space-y-2">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-[0.2em]">CropIQ টুলস</p>
            <h2 className="text-xl md:text-2xl font-extrabold text-gray-800">আপনার প্রয়োজনীয় টুল বেছে নিন</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
            {allTools.map(tool => (
              <Link
                key={tool.href}
                href={tool.href}
                className="group relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 md:p-7 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
              >
                ...
                <div className="relative space-y-4">
                  {/* Icon */}
                  <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${tool.color} flex items-center justify-center shadow-md group-hover:scale-110 transition-transform duration-300`}>
                    <tool.icon className="w-7 h-7 text-white" />
                  </div>

                  {/* Text */}
                  <div>
                    <h3 className="text-lg font-extrabold text-gray-800 mb-1">{tool.label}</h3>
                    <p className="text-sm text-gray-500 leading-relaxed">{tool.desc}</p>
                  </div>

                  {/* CTA */}
                  <div className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r ${tool.color} text-white text-sm font-bold shadow-md group-hover:shadow-lg group-hover:gap-3 transition-all`}>
