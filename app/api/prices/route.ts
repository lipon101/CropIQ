...
export async function GET(req: NextRequest) {
  try {
    ...
    let source: string
    if (isNational) {
      // Full transparency: show every live record (DAM national + all WFP
      // district-level rows) instead of collapsing WFP into one national
      // average per commodity — so the board proves it holds all 728+ real
      // rows, not just a shortlist of national averages.
      rows = [...dam, ...wfp].sort((a, b) => b.price_per_kg - a.price_per_kg)
      source = "dam"
    } else {
